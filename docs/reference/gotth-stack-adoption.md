...
| `gotth-extensions` | `3822dd722bc8606b8843d0a68c7c3e9f598d9ccc` | Adopted on the 1.0-alpha development line for bounded manifest/grant validation, exact subset negotiation, canonical digests, lifecycle validation, and the handshake/health control contract. GOTTH Mail owns installed instances, transport credentials, secret values, supervision, routing, every seam RPC, product authorization, mutation, audit, update, and rollback policy. The foundation is not a generic invocation API or cross-product controller. |
