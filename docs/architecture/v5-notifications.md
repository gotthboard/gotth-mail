...
This slice is a standalone plugin adapter. The control-plane application still uses the default first-mechanism registry and has no alert-dispatch selection path for the signed-email registration. Core routing, opt-in selection, and composition with the SQL delivery recorder remain required before this can be described as an application notification path.
...
This slice does not implement control-plane routing/selection, per-user/role/delegation notification identity selection, message-context authorization, public-key discovery, or full rotation/deletion/recovery policy. Those remain explicit feature blockers; the standalone system-identity adapter must not be used to claim application delivery, user identity, or full profile conformance.
