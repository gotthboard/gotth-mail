...
Canonical desired state lives under `deploy/authentik/gotth-mail/`:

- `manifest.json` is the reviewed, secret-free GOTTH Mail consumer input;
- `blueprint.yaml` is generated byte-for-byte by `gotth-authentik` revision
  `77c811c18fe3c107f6f3e97ce3f9a3850a685300`;
- `scripts/verify-authentik-profile.sh` checks the pinned renderer revision,
  deterministic output, secret absence, and expected identifiers.

The profile declares application slug `gotth-mail`, provider
`gotth-mail-oidc`, client ID `gotth-mail`, access group `gotth-mail-users`,
and strict callback `http://127.0.0.1:18080/api/v1/oidc/callback`. That callback
is deliberately classified as a pre-production loopback smoke target. A
production deployment must render and review a manifest with the exact public
HTTPS callback; permissive, wildcard, or inferred callbacks are forbidden.

The rollout sequence is fixed:

1. capture a root-readable Authentik PostgreSQL backup and a redacted inventory
   of the existing application/provider/bindings;
2. validate the generated blueprint with Authentik 2026.5.2;
3. import it alongside the historical `gophermailforge` objects;
4. add only explicitly reviewed users to the application access group;
5. prove exact issuer, JWKS, authorization redirect, state, nonce, client ID,
   strict callback, and secret preservation across re-import;
6. supply the generated client secret to a GOTTH Mail runtime only through a
   root-readable file and run the real browser callback proof;
7. configure and prove the separate Authentik SCIM provider and full
   provision/login/disable/deprovision lifecycle; and
8. retire the historical objects only after restart, backup, isolated restore,
   and rollback evidence passes.

The imported access group is not authorization. It only gates entry to the
application. It creates no `role_bindings` row, no mailbox, no SCIM resource,
and no trusted group claim. The current renderer does not own remote apply,
secret distribution, SCIM provider configuration, or product roles.
