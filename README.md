...
- **Identity provisioning (`v2` historical ID):** the duplicate in-tree OIDC protocol implementation has been replaced by the pinned `gotth-oidc` consumer boundary. Protected PKCE attempts and application sessions use the configured PostgreSQL database, and startup refuses an OIDC configuration without durable storage. This remains an unadmitted feature branch until PostgreSQL, restart, concurrency, full/race, review, and live Authentik evidence pass. The handwritten SCIM surface still must be replaced by `gotth-scim`; durable role mapping, opaque SCIM-ID adoption, and live provisioning/deprovisioning proof remain required.
