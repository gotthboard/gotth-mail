...
Canonical root implementation worktree locations must be under the durable `/tank/development/linus/gophermailforge-worktrees` path recorded in `workflow.toml`. Volatile `/tmp` paths may be compatibility symlinks only; they are not canonical storage. Every meaningful repository change must update [docs/CHANGELOG.md](docs/CHANGELOG.md) in the same commit.
