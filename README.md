...
- **Same-domain-only outbound policy (owner priority):** a planned per-domain
  option will restrict every mailbox and system sender in that domain to the
  ...
  transport; uncertainty defers rather than permits delivery. It is the next
  implementation assignment after the active live-identity boundary and blocks
  alpha integration.
