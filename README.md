...
- **Identity provisioning (`v2` historical ID):** the duplicate OIDC and SCIM protocol implementations have been replaced on the 1.0-alpha development line by exact pinned `gotth-oidc` and `gotth-scim` consumer boundaries. Protected PKCE attempts, application sessions, SCIM resources, indexes, tombstones, mailboxes, verifier projections, and audit records use PostgreSQL. App passwords now retain distinct opaque public IDs and human labels across restart, commit create/revoke with their success audits, cap active PBKDF2 verifiers at eight per mailbox, and fail closed when accepted passdb use cannot be audited. The old browser SCIM shortcut and unauthenticatable app-password forms are removed; browser self-service waits for authoritative OIDC subject/mailbox/role binding. SCIM Groups remain deliberately disabled until opaque Authentik subject/member binding is admitted. Legacy email-keyed identities still need an explicit operator-reviewed adoption map, and live Authentik provisioning/deprovisioning, restart, backup, and restore evidence remains required before this workstream can close.
