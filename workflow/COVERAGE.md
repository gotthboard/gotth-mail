...
| Webmail | IMAP/SMTP integration, MIME/XSS/CSP, attachment safety | v4 webmail client/sender unit tests and authenticated mailbox-scoped API wiring tests cover folder/list/search/read, draft submit, request-body From rejection/binding, resolver-before-signer exact-sender seam, real OpenPGP/MIME signing and exact-sender verification with generated keys, MIME header injection rejection, non-colliding draft IDs, bounded search, escaped hostile HTML with explicit text-only rendering decision, CSP, attachment safety, bounded raw MIME parsing with hostile fixtures, SQL-backed mailbox-owned draft persistence/reload, SQL-backed submit state persistence, SQL-backed reply/forward linkage and attachment payload durability, API wiring to the configured SQL draft store, real SMTP transport unit tests, real IMAP transport unit tests, custom `/webmail` shell reachability, and repo-owned containerized Postfix/Dovecot/Rspamd SMTP+IMAP plus webmail UI smokes in `scripts/containerized-webmail-smtp-smoke.sh`, `scripts/containerized-webmail-imap-smoke.sh`, and `scripts/containerized-webmail-ui-smoke.sh` | production OpenPGP key storage/unlock lifecycle remains a deployment concern; draft submit ownership hole, fake-only SMTP/IMAP transports, metadata-only draft persistence, raw MIME fixture gap, custom webmail UI/container reachability, fake OpenPGP structure checking, and vague rich-HTML sanitizer requirement repaired/narrowed |
| Notifications | plugin gRPC, Telegram redaction, actor mapping, approval binding | local v5 alert-core tests in `internal/notification` cover sanitized bounded alert payloads, secret redaction before backend delivery, pending/final delivery-status recording, retryable failure visibility, SQL-backed delivery status persistence, explicit SQL actor mapping, Telegram update receiver command/callback routing, read-only command dispatch/audit, and approval replay/mismatch/expiry protections; `internal/notifyruntime` covers real bounded summaries from doctor, queue, domain, backup, deployment, plugin state, and narrow approved queue mutation execution; `internal/api` covers the configured SQL delivery-status read API; `internal/plugin` and `scripts/containerized-notification-plugin-smoke.sh` cover the notification plugin container on the authenticated gRPC control seam plus notification-specific SendAlert/SendPrompt RPCs, including wrong-token rejection and unsafe payload rejection | live Telegram delivery proof and OpenPGP-signed email notifications remain pending |
...
Remaining production integrations are not done and must not be represented as done: live Authentik, production IMAP/SMTP, OpenPGP crypto/canonicalization, raw MIME parsing, rich HTML sanitizer/browser proof, Telegram delivery/plugin runtime, approval binding, and remaining durable runtime wiring.
