...
| Backup/import/rollback | isolated restore, import preview/apply binding, no silent weakening | v3 authenticated operator API tests, SQL-backed audit query/detail/retention tests, durable SQL backup verification record tests, no forged snapshot lookup/diff tests, Mailu preview/apply hash/fingerprint/actor validation, stricter candidate/verifier validation, and audit-durable bulk apply tests in `internal/api`/`internal/ops` | backup verification records are durable for SQL routes but restore execution remains modeled; live isolated restore and live Mailu import compatibility still pending; canonical durable bulk/snapshot storage still pending; UI/API auth bypasses repaired and fake UI backup success removed |
