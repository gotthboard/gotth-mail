...
| Extensions foundation | immutable dependency pin, strict manifest/grant negotiation, lifecycle, authenticated handshake/health, hostile binding, failure isolation, existing seam compatibility | `internal/plugin/foundation_test.go`, `test/contract/gotth_extensions_contract_test.go`, configured notification runtime tests, and the containerized live-plugin smoke prove the reviewed `gotth-extensions` consumer boundary without replacing Mail-owned seam RPCs | independent extension distribution and a production supervisor/configuration transport remain external to the shared compatibility foundation |
| Extensions administrator | durable inventory, encrypted write-only secrets, host-owned metadata/UI, actor/revision-bound previews, lifecycle ordering, update diffs, rollback, uninstall isolation | `internal/extensionsadmin`, `internal/extensionsruntime`, `cmd/gotth-mail/extensions_env_test.go`, `internal/api/extensions_test.go`, `internal/httpui/extensions_test.go`, `internal/authn/sql_store_test.go`, and migration parity/real-PostgreSQL tests cover closed metadata, no secret rendering/persistence in previews or audit, exact runtime ordering and failure isolation, durable OIDC-role plus CSRF admission, cross-product rejection, full previous-version rollback, separate secret deletion/uninstall, protected artifact/runtime roots, a real independently built webhook child process, full authenticated handshake/health, routing order, bounded transport failure, confirmed stop, and runtime-secret cleanup | public GitHub distribution, immutable webhook release/tag parity, deployed successful HTTPS delivery, physical accessibility, and crash/restart reconciliation remain release/acceptance gates |
...
| Admin UI | native ordinary-HTML/API service path integration, no bypass | v3 operator routes, session-bound app-password self-service, and the Extensions administrator cover authenticated service paths, durable OIDC-role admission, missing-session/CSRF rejection, responsive keyboard-focusable host-owned controls, write-only secrets, conditional route/link exposure, one-time secret display, verifier redaction, and revocation. | live Authentik browser proof and physical mobile/theme accessibility acceptance remain deployment/owner gates |
...
Publish and pin one independently distributed `gotth-extension-<slug>` artifact
with a documented local supervisor/configuration transport, then bind the
already-implemented administrator runtime interface to it and run the complete
setup/test/enable/disable/update/rollback/uninstall path against the real
artifact. Do not pass stored secrets through process arguments or pretend that
an already-running plugin consumed new configuration.

After that, migrate the installed Authentik provider/profile from the historical
GopherMailForge namespace to GOTTH Mail and run live browser/passkey OIDC plus
installed SCIM lifecycle proof. The repository-local same-domain policy,
webmail, and notification implementation slices are complete; remaining live
operator-owned credentials, registrations, artifact publication, and physical
acceptance must remain explicit alpha blockers rather than fixtures.
