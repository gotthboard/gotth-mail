...
PROJECT=${GMF_WEBMAIL_IMAP_SMOKE_PROJECT:-gmf-webmail-imap-smoke}
...
trap cleanup EXIT
$DOCKER compose -p "$PROJECT" -f "$COMPOSE" down -v --remove-orphans >/dev/null 2>&1 || true
$DOCKER compose -p "$PROJECT" -f "$COMPOSE" up -d --build gophermailforge postfix dovecot rspamd
wait_tcp() {
  host=$1
  port=$2
...
subject="GopherMailForge webmail IMAP smoke $(date +%s)"
$DOCKER compose -p "$PROJECT" -f "$COMPOSE" build test-runner
$DOCKER compose -p "$PROJECT" -f "$COMPOSE" run --rm -T --no-deps \
  -e GMF_LIVE_SMTP_ADDR=postfix:25 \
  -e "GMF_LIVE_SMTP_SUBJECT=$subject" \
...
$DOCKER compose -p "$PROJECT" -f "$COMPOSE" run --rm -T --no-deps \
  -e GMF_LIVE_IMAP_ADDR=dovecot:143 \
  -e GMF_LIVE_IMAP_USER=smoke@example.test \
  -e GMF_LIVE_IMAP_PASSWORD=smoke-secret \
  -e "GMF_LIVE_IMAP_SUBJECT=$subject" \
