...
func TestApprovalServiceCreatesAndDeliversBoundPromptWithoutReturningSecret(t *testing.T) {
	...
	created, err := service.RequestTelegramApproval(context.Background(), TelegramApprovalRequest{TransportActor: transportActor, Initiator: actor, Action: "queue:flush", Resource: authz.Resource{Type: "queue", ID: "default"}, CorrelationID: "corr-1", ExpiresAt: now.Add(time.Minute), Title: "Approve flush", Summary: "Flush deferred mail"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApprovalServiceInvalidatesPromptWhenDeliveryFails(t *testing.T) {
	...
	if _, err := service.RequestTelegramApproval(context.Background(), TelegramApprovalRequest{TransportActor: transportActor, Initiator: actor, Action: "queue:retry", Resource: authz.Resource{Type: "queue", ID: "default"}, CorrelationID: "corr-2", ExpiresAt: now.Add(time.Minute), Title: "Approve retry", Summary: "Retry deferred mail"}); err == nil {
...
func TestApprovalServiceRequiresInitiatorBeforeMappingOrSnapshot(t *testing.T) {
	db := testpg.DB(t, store.MigrateSQL)
	prompter := &capturePrompter{}
	service := ApprovalService{
		Mapper:     notification.SQLActorMapper{DB: db},
		Authorizer: authz.StaticAuthorizer{},
		Store:      notification.SQLApprovalStore{DB: db},
		Prompter:   prompter,
		Queue:      &fakeQueueController{snapshot: outboundpolicy.QueueSnapshot{Digest: strings.Repeat("c", 64)}},
	}
	_, err := service.RequestTelegramApproval(context.Background(), TelegramApprovalRequest{
		TransportActor: notification.TransportActor{Transport: "telegram", ExternalID: "chat:42:user:99"},
		Action:         "queue:flush",
		Resource:       authz.Resource{Type: "queue", ID: "default"},
		ExpiresAt:      time.Now().Add(time.Minute),
	})
	if err == nil || !strings.Contains(err.Error(), "initiator") {
		t.Fatalf("missing initiator accepted: %v", err)
	}
	if prompter.prompt.ID != "" {
		t.Fatalf("prompt sent for invalid initiator: %#v", prompter.prompt)
	}
}
