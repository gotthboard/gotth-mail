...
func (s SQLRecorder) RecordPending(ctx context.Context, a Alert, now time.Time) error {
	...
	_, err = s.DB.ExecContext(ctx, `INSERT INTO notification_deliveries(alert_id, alert_json, status, reason, created_at, updated_at) VALUES ($1,$2,$3,'',$4,$4) ON CONFLICT (alert_id) DO UPDATE SET alert_json=EXCLUDED.alert_json, status=EXCLUDED.status, reason='', updated_at=EXCLUDED.updated_at`, clean.ID, string(payload), StatusPending, now.UTC())
	return err
}

func (s SQLRecorder) RecordFinal(ctx context.Context, id string, status DeliveryStatus, reason string, now time.Time) error {
	if s.DB == nil {
		return errors.New("notification delivery db required")
	}
	...
	res, err := s.DB.ExecContext(ctx, `UPDATE notification_deliveries SET status=$2, reason=$3, updated_at=$4 WHERE alert_id=$1`, id, status, bound(reason, 256), now.UTC())
	if err != nil {
		return err
	}
...
func (s SQLRecorder) Get(ctx context.Context, id string) (DeliveryRecord, bool, error) {
	...
	var payload string
	err := s.DB.QueryRowContext(ctx, `SELECT alert_json, status, reason, created_at, updated_at FROM notification_deliveries WHERE alert_id=$1`, id).Scan(&payload, &rec.Status, &rec.Reason, &rec.CreatedAt, &rec.UpdatedAt)
...
func (s SQLRecorder) List(ctx context.Context) ([]DeliveryRecord, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `SELECT alert_json, status, reason, created_at, updated_at FROM notification_deliveries ORDER BY updated_at DESC, alert_id ASC LIMIT 200`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var rec DeliveryRecord
		var payload string
		if err := rows.Scan(&payload, &rec.Status, &rec.Reason, &rec.CreatedAt, &rec.UpdatedAt); err != nil {
			return nil, err
		}
		if err := json.Unmarshal([]byte(payload), &rec.Alert); err != nil {
			return nil, err
