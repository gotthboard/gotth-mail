...
func TestSQLApprovalStoreAcceptsExactBindingOnce(t *testing.T) {
	...
	approved, err := store.Confirm(context.Background(), confirmationFixture(now))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.Confirm(context.Background(), confirmationFixture(now)); err == nil || !strings.Contains(err.Error(), "replay") {
...
func TestSQLApprovalStoreRejectsMismatchAndExpiry(t *testing.T) {
	...
	if _, err := store.Create(context.Background(), approvalFixture(now), now); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || !ok || got.Result != "rejected" || got.UsedAt != nil {
		t.Fatalf("mismatch did not fail closed: got=%#v ok=%v err=%v", got, ok, err)
	}

	expired := approvalFixture(now)
	expired.ID = "approval-2"
	if _, err := store.Create(context.Background(), expired, now); err != nil {
		t.Fatal(err)
	}
