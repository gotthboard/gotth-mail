...
.reader-pane{padding-bottom:2rem}.empty-state{height:100%;display:grid;place-content:center;text-align:center;gap:.5rem;color:var(--muted);padding:2rem}.empty-state strong{font-size:18px;color:var(--text)}.message-header{padding:1rem 1.2rem;border-bottom:1px solid var(--line)}.message-header h2{margin:0 0 .8rem;font-size:20px}.message-header dl{display:grid;grid-template-columns:max-content 1fr;gap:.2rem .65rem;margin:0}.message-header dt{font-weight:700}.message-header dd{margin:0;overflow-wrap:anywhere}.message-body{margin:0;padding:1.2rem;white-space:pre-wrap;overflow-wrap:anywhere;font:14px/1.55 ui-monospace,SFMono-Regular,Consolas,monospace}.reader-pane h3,#attachment-list{margin-left:1.2rem;margin-right:1.2rem}
...
function renderMessages(){var list=byId('message-list');list.replaceChildren();sortedMessages().forEach(function(message){var row=document.createElement('button');row.type='button';row.className='message-row';if(!hasFlag(message,'\\Seen'))row.classList.add('unread');if(hasFlag(message,'\\Flagged'))row.classList.add('flagged');if(message.HasAttachments)row.classList.add('has-attachment');row.dataset.id=message.ID;row.setAttribute('role','option');row.setAttribute('aria-selected',String(message.ID===state.selected));var from=document.createElement('span');from.className='from';from.textContent=message.From||'(unknown sender)';var subject=document.createElement('span');subject.className='subject';subject.textContent=message.Subject||'(no subject)';var date=document.createElement('span');date.className='date';date.textContent=displayDate(message.Date);row.append(from,subject,date);row.addEventListener('click',function(){openMessage(message.ID)});row.addEventListener('contextmenu',function(event){event.preventDefault();state.selected=message.ID;renderMessages();openContext(event.clientX,event.clientY)});row.addEventListener('keydown',function(event){if(event.key==='F10'&&event.shiftKey){event.preventDefault();state.selected=message.ID;renderMessages();var box=row.getBoundingClientRect();openContext(box.left+20,box.top+20)}});list.appendChild(row)});list.setAttribute('aria-busy','false');byId('load-more').hidden=!state.next;updateCommands()}
...
async function openMessage(id){try{state.selected=id;renderMessages();var message=await api('/api/v1/webmail/message?folder='+encode(state.folder)+'&id='+encode(id));state.context=message;byId('empty-reader').hidden=true;byId('message-reader').hidden=false;byId('message-subject').textContent=message.Subject||'(no subject)';byId('message-from').textContent=message.From||'';byId('message-to').textContent=message.To||'';byId('message-cc').textContent=message.Cc||'';byId('message-date').textContent=displayDate(message.Date);byId('message-body').textContent=message.BodyText||message.BodyHTML||'';renderAttachments(message);document.body.dataset.mobileView='reader';byId('reader-pane').focus();setStatus('Opened '+(message.Subject||'message'),false);if(!hasFlag(selectedSummary()||{},'\\Seen'))await messageAction('mark_read',null,true)}catch(error){handleError(error)}}
function renderAttachments(message){var section=byId('attachment-section');var list=byId('attachment-list');list.replaceChildren();var attachments=message.Attachments||[];section.hidden=attachments.length===0;attachments.forEach(function(attachment,index){var item=document.createElement('li');var link=document.createElement('a');link.textContent=(attachment.Filename||'attachment')+' ('+(attachment.Size||0)+' bytes)';link.href='/api/v1/webmail/attachment?folder='+encode(state.folder)+'&id='+encode(message.ID)+'&index='+index;item.appendChild(link);list.appendChild(item)})}
...
async function messageAction(action,destination,quiet){if(!state.selected)return;try{var body={action:action};if(destination)body.destination=destination;await api('/api/v1/webmail/message?folder='+encode(state.folder)+'&id='+encode(state.selected),{method:'POST',body:JSON.stringify(body)});if(action==='delete'||action==='move'){state.selected=null;state.context=null;byId('message-reader').hidden=true;byId('empty-reader').hidden=false}await loadMessages(false);if(!quiet)setStatus('Message action completed',false)}catch(error){handleError(error)}}
