...
func TestNotificationDeliveryEvidenceMigrationFileMatchesRuntime(t *testing.T) {
	...
	if len(upgradeMigrations) != 5 || upgradeMigrations[0].Version != notificationDeliveryEvidenceMigrationVersion || upgradeMigrations[0].SQL != fileSQL {
...
func TestOIDCProtectedAttemptsMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 5 || upgradeMigrations[1].Version != oidcProtectedAttemptsMigrationVersion || upgradeMigrations[1].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestSCIMResourcesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 5 || upgradeMigrations[2].Version != scimResourcesMigrationVersion || upgradeMigrations[2].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestAppPasswordContractMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 5 || upgradeMigrations[3].Version != appPasswordContractMigrationVersion || upgradeMigrations[3].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOIDCSCIMIdentityBindingMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 5 || upgradeMigrations[4].Version != oidcSCIMIdentityBindingMigrationVersion || upgradeMigrations[4].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
