...
func TestNotificationDeliveryEvidenceMigrationFileMatchesRuntime(t *testing.T) {
	...
	if len(upgradeMigrations) != 15 || upgradeMigrations[0].Version != notificationDeliveryEvidenceMigrationVersion || upgradeMigrations[0].SQL != fileSQL {
...
func TestOIDCProtectedAttemptsMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[1].Version != oidcProtectedAttemptsMigrationVersion || upgradeMigrations[1].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestSCIMResourcesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[2].Version != scimResourcesMigrationVersion || upgradeMigrations[2].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestAppPasswordContractMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[3].Version != appPasswordContractMigrationVersion || upgradeMigrations[3].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOIDCSCIMIdentityBindingMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[4].Version != oidcSCIMIdentityBindingMigrationVersion || upgradeMigrations[4].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestSCIMGroupMembersMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[5].Version != scimGroupMembersMigrationVersion || upgradeMigrations[5].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundPolicyMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[6].Version != outboundPolicyMigrationVersion || upgradeMigrations[6].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundQueueMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[7].Version != outboundQueueMigrationVersion || upgradeMigrations[7].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundQueueReleaseMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[8].Version != outboundQueueReleaseMigrationVersion || upgradeMigrations[8].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestWebmailCCBCCMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[9].Version != webmailCCBCCMigrationVersion || upgradeMigrations[9].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestWebmailDeliveryUncertainMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[10].Version != webmailDeliveryUncertainMigrationVersion || upgradeMigrations[10].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationApprovalBindingMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[11].Version != notificationApprovalBindingMigrationVersion || upgradeMigrations[11].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationApprovalExecutionMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[12].Version != notificationApprovalExecutionMigrationVersion || upgradeMigrations[12].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationTelegramUpdatesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[13].Version != notificationTelegramUpdatesMigrationVersion || upgradeMigrations[13].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationEventStatesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 15 || upgradeMigrations[14].Version != notificationEventStatesMigrationVersion || upgradeMigrations[14].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
