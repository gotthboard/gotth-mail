...
func TestNotificationDeliveryEvidenceMigrationFileMatchesRuntime(t *testing.T) {
	...
	if len(upgradeMigrations) != 17 || upgradeMigrations[0].Version != notificationDeliveryEvidenceMigrationVersion || upgradeMigrations[0].SQL != fileSQL {
...
func TestOIDCProtectedAttemptsMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[1].Version != oidcProtectedAttemptsMigrationVersion || upgradeMigrations[1].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestSCIMResourcesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[2].Version != scimResourcesMigrationVersion || upgradeMigrations[2].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestAppPasswordContractMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[3].Version != appPasswordContractMigrationVersion || upgradeMigrations[3].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOIDCSCIMIdentityBindingMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[4].Version != oidcSCIMIdentityBindingMigrationVersion || upgradeMigrations[4].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestSCIMGroupMembersMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[5].Version != scimGroupMembersMigrationVersion || upgradeMigrations[5].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundPolicyMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[6].Version != outboundPolicyMigrationVersion || upgradeMigrations[6].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundQueueMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[7].Version != outboundQueueMigrationVersion || upgradeMigrations[7].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestOutboundQueueReleaseMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[8].Version != outboundQueueReleaseMigrationVersion || upgradeMigrations[8].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestWebmailCCBCCMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[9].Version != webmailCCBCCMigrationVersion || upgradeMigrations[9].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestWebmailDeliveryUncertainMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[10].Version != webmailDeliveryUncertainMigrationVersion || upgradeMigrations[10].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationApprovalBindingMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[11].Version != notificationApprovalBindingMigrationVersion || upgradeMigrations[11].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationApprovalExecutionMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[12].Version != notificationApprovalExecutionMigrationVersion || upgradeMigrations[12].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationTelegramUpdatesMigrationFileMatchesRuntime(t *testing.T) {
	...
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[13].Version != notificationTelegramUpdatesMigrationVersion || upgradeMigrations[13].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
...
func TestNotificationEventStatesMigrationFileMatchesRuntime(t *testing.T) {
	data, err := os.ReadFile("../../migrations/0016_notification_event_states.sql")
	if err != nil {
		t.Fatal(err)
	}
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[14].Version != notificationEventStatesMigrationVersion || upgradeMigrations[14].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
	if got, want := upgradeMigrations[14].Checksum, hex.EncodeToString(sum[:]); got != want {
		t.Fatalf("runtime checksum=%q file checksum=%q", got, want)
	}
}

func TestExtensionAdministratorMigrationFileMatchesRuntime(t *testing.T) {
	data, err := os.ReadFile("../../migrations/0017_extension_administrator.sql")
	if err != nil {
		t.Fatal(err)
	}
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[15].Version != extensionAdministratorMigrationVersion || upgradeMigrations[15].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
	if got, want := upgradeMigrations[15].Checksum, hex.EncodeToString(sum[:]); got != want {
		t.Fatalf("runtime checksum=%q file checksum=%q", got, want)
	}
}

func TestRoleBindingAuthorityMigrationFileMatchesRuntime(t *testing.T) {
	data, err := os.ReadFile("../../migrations/0018_role_binding_authority.sql")
	if err != nil {
		t.Fatal(err)
	}
	fileSQL := strings.TrimSpace(string(data))
	if len(upgradeMigrations) != 17 || upgradeMigrations[16].Version != roleBindingAuthorityMigrationVersion || upgradeMigrations[16].SQL != fileSQL {
		t.Fatalf("runtime migration registration drift: %#v", upgradeMigrations)
	}
	sum := sha256.Sum256([]byte(fileSQL))
	if got, want := upgradeMigrations[16].Checksum, hex.EncodeToString(sum[:]); got != want {
		t.Fatalf("runtime checksum=%q file checksum=%q", got, want)
	}
}

func TestRoleBindingAuthorityMigrationRejectsAmbiguousExistingRows(t *testing.T) {
	for _, test := range []struct {
		name string
		rows string
	}{
		{"inconsistent domain", `INSERT INTO role_bindings VALUES ('00000000-0000-4000-8000-000000000301','00000000-0000-4000-8000-000000000302','global_admin','00000000-0000-4000-8000-000000000303',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`},
		{"duplicate global", `INSERT INTO role_bindings VALUES ('00000000-0000-4000-8000-000000000304','00000000-0000-4000-8000-000000000302','global_admin',NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP), ('00000000-0000-4000-8000-000000000305','00000000-0000-4000-8000-000000000302','global_admin',NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`},
	} {
		t.Run(test.name, func(t *testing.T) {
			db := testpg.DB(t, func(ctx context.Context, db *sql.DB) error {
				_, err := db.ExecContext(ctx, `CREATE TABLE role_bindings (id uuid primary key, identity_ref_id uuid not null, role text not null, domain_id uuid null, created_at timestamp not null, updated_at timestamp not null)`)
				return err
			})
			if _, err := db.Exec(test.rows); err != nil {
				t.Fatal(err)
			}
			if _, err := db.Exec(roleBindingAuthorityMigrationSQL); err == nil {
				t.Fatal("ambiguous existing authority passed migration")
			}
		})
	}
}
