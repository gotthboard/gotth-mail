...
USER gotthmail
RUN go test ./... && go build -o /out/gotth-mail ./cmd/gotth-mail && go build -o /out/gotth-mailctl ./cmd/gotth-mailctl && go build -o /out/gotth-mail-plugin ./cmd/gotth-mail-plugin && go build -o /out/gotth-mail-postfix-gate ./cmd/gotth-mail-postfix-gate
FROM alpine:3.22
...
COPY --from=build /out/gotth-mail-postfix-gate /usr/local/bin/gotth-mail-postfix-gate
EXPOSE 8080 9443 10025
